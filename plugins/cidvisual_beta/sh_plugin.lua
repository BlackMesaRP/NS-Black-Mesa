...
nut.lang.Add("vcid_retrived", "You have retrived %s's CID from someone in front of you.")
	...
	nut.command.Register({
		...
		onRun = function(client, arguments)
			...
			if (IsValid(entity) and entity:IsPlayer() and entity.character) then
				...
				if !client:GetItem( "cid2" ) then
				nut.util.Notify(  nut.lang.Get( "vcid_nocid", cidname ) , client ) return end
				for k,v in pairs( client:GetItem( "cid2" ) ) do
					if k == "data" then
						if v.Digits and v.Owner then
							entity:ShowCID( client, v.Digits, v.Owner, v.Model, v.Forged )
							return
						end
					end
				end
